...
class TournamentLogoService
{
    private const MAX_BYTES = 5 * 1024 * 1024;
    private const MAX_DIMENSION = 1000;
    private const ALLOWED_MIMES = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
    private const UPLOAD_SUBDIR = 'uploads/tournament_logos';

    public function validateUpload(array $file): void
    {
        if (!isset($file['tmp_name'], $file['error'])) {
            throw new RuntimeException('Please upload a tournament logo.');
        }

        $error = (int)$file['error'];
        if ($error !== UPLOAD_ERR_OK) {
            $message = match ($error) {
                UPLOAD_ERR_INI_SIZE, UPLOAD_ERR_FORM_SIZE => 'Logo file is too large. Max file size is 5MB.',
                UPLOAD_ERR_PARTIAL => 'Logo upload was interrupted. Please try again.',
                UPLOAD_ERR_NO_FILE => 'Please upload a tournament logo.',
        ...
        if (($file['size'] ?? 0) > self::MAX_BYTES) {
            throw new RuntimeException('Logo file is too large. Max file size is 5MB.');
        }
    ...
    /**
     * @return string Web-relative path e.g. uploads/tournament_logos/3_abc.png
     */
    public function saveForTournament(array $file, int $tournamentId): string
    {
        ...
        if (!is_dir($uploadDir) && !mkdir($uploadDir, 0755, true) && !is_dir($uploadDir)) {
            throw new RuntimeException('Failed to prepare logo upload directory.');
        }
