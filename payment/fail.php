...
if ($tranId !== '') {
    try {
        ...
        if ($registration) {
            ...
            $repo->updatePaymentStatus($regType, $tranId, 'failed');
        } else {
            $targetTable = ($regType === 'non_golfer') ? 'registrations_non_golfer' : 'registrations';
            db()->prepare("UPDATE {$targetTable} SET payment_status = 'failed' WHERE tran_id = ? AND payment_status = 'pending'")
                ->execute([$tranId]);
        }
    } catch (Throwable $e) {
        appLog('[payment/fail.php] DB status update failed', ['error' => $e->getMessage(), 'tran_id' => $tranId]);
    }
}
